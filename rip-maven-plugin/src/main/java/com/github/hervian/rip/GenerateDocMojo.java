...
//TODO: Clean up this method
  ...
  private File generateSwaggerDoc() throws MojoExecutionException {
    DocumentGeneratorInput input = DocumentGeneratorInput
      ...
      .outputDir(getSwaggerDocDir(project))
      .build();
    ...
    getLog().info("openApiDocumentGenerator = " + openApiDocumentGenerator.getClass().getSimpleName());
    openApiDocumentGenerator.generate(input);
    File swaggerFile = new File(getSwaggerDocFileAbsolutePath(project));
    return swaggerFile;
  }
  ...
  private boolean isSpringWithJaxRsProject() {
    ...
    boolean isSpringWithJaxRsProject = true;
    if (generateDocConfig.getRestAnnotationType()==null || generateDocConfig.getRestAnnotationType()==GenerateDocConfig.RestAnnotationType.UNSPECIFIED) {
      String fqcnOfRequestMappingClass = "org.springframework.web.bind.annotation.RequestMapping";
      getLog().info("The plugin parameter generateDocConfig.getRestAnnotationType() is null or UNSPECIFIED. The rip-maven-plugin will try to determine which Rest controller annotations to use by instantiating the class " + fqcnOfRequestMappingClass);
      isSpringWithJaxRsProject = !ClassPathHelper.isOnClassPath(fqcnOfRequestMappingClass);
    } else {
      getLog().info("generateDocConfig.getRestAnnotationType() = " + generateDocConfig.getRestAnnotationType());
      isSpringWithJaxRsProject = generateDocConfig.getRestAnnotationType() == GenerateDocConfig.RestAnnotationType.SPRING_JAX_RS;
    }
    getLog().info("isSpringWithJaxRsProject: " + isSpringWithJaxRsProject + " (Meaning: Whether the autogenerated resources classes that serves the swagger.json and the swagger-ui.html will be made with standard JAX-RS annotations or Spring MVN annotations.)");
    return isSpringWithJaxRsProject;*/
  }
  ...
  private void copySwaggerDocToBuildOutputDir(String fileName, String destinationFileName) throws MojoExecutionException {
    try {
      File swaggerFile = new File(getSwaggerDocDir(project)+"/" + fileName);
  ...
  static String getSwaggerDocFileAbsolutePath(MavenProject project) {
    return getSwaggerDocDir(project)+"/" + openapiDocFileName;
  }

  private static String getSwaggerDocDir(MavenProject project){
